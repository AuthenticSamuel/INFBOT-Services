...
module.exports = async (client) => {
    ...
    client.guilds.cache.forEach(async guild => {
        ...
        /*
        try {await connection.query(`INSERT INTO guilds (guildId) VALUES('${guild.id}')`)} catch {console.error}
        try {await connection.query(`INSERT INTO utils (guildId) VALUES('${guild.id}')`)} catch {console.error}
        try {await connection.query(`INSERT INTO voice (guildId)VALUES('${guild.id}')`)} catch {console.error}
        */
    });
    ...
    try {
        client.guilds.cache.forEach(async (guild) => {
            await connection.query(`SELECT * FROM utils WHERE guildId = '${guild.id}'`).then(result => {
            ...
            await connection.query(`SELECT channelId FROM voiceChannels WHERE guildId = '${guild.id}'`).then(result => {
                if (result[0]) {
